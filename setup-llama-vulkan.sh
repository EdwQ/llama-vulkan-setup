...
echo "============================================"
echo ""
echo "1. 安装 ModelScope CLI (推荐方式):"
echo "   pip install modelscope"
echo ""
echo "2. 下载推荐模型 (Qwen3.6 27B Q4_K_M):"
echo "   # 方式 A: 使用 modelscope-cli (推荐)"
echo "   mkdir -p ~/llama-models"
echo "   cd ~/llama-models"
echo "   python -c \"from modelscope import snapshot_download; snapshot_download('unsloth/Qwen3.6-27B-GGUF', allow_patterns='*Q4_K_M*')\""
echo ""
echo "   # 方式 B: 直接下载 (无需安装 modelscope)"
echo "   mkdir -p ~/llama-models"
echo "   cd ~/llama-models"
...
echo "   ./llama-cli -m ~/llama-models/Qwen3.6-27B-Instruct-Q4_K_M.gguf -p \"你好\" -n 100 -ngl 99"
echo ""
echo "4. 启动 HTTP 服务器模式:"
echo "   cd ~/llama.cpp/build/bin"
echo "   ./llama-server -m ~/llama-models/Qwen3.6-27B-Instruct-Q4_K_M.gguf -ngl 99 -c 4096 --host 0.0.0.0 --port 8080"
echo ""
echo "5. 访问 API (浏览器或 curl):"
...
echo "- 推荐模型：Qwen3.6-27B-IQ4_NL.gguf (模型约 15GB + KV Cache)"
echo ""
echo "  上下文长度显存需求参考 (27B 模型，Q4 量化):"
echo "  - -c 2048  → 约 16GB 总显存 (适合短对话)"
echo "  - -c 4096  → 约 17GB 总显存 (推荐)"
echo "  - -c 8192  → 约 19GB 总显存 (长文档)"
echo "  - -c 16384 → 约 23GB 总显存 (需大显存)"
echo ""
echo "  不同显存推荐配置:"
echo "  - 16GB 显卡：-c 2048 (短对话)"
echo "  - 20GB 显卡：-c 4096 (推荐)"
echo "  - 24GB 显卡：-c 8192 (长文档)"
echo "  - 32GB 显卡：-c 16384 或更高 (超长上下文)"
echo ""
echo "- 更多 GGUF 模型: https://huggingface.co/models?library=gguf"
echo "- 直接下载：https://www.modelscope.cn/models/unsloth/Qwen3.6-27B-GGUF"
echo ""
